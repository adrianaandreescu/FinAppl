...
import { Data } from '../../pageObjects/balanta-contabila-PO';

let balanceSheet = new Data();
...
describe('should test the "Introducere date" section with all its functionalities', function(){
    ...
    it('should upload BalanceSheet', async()=>{
        await balanceSheet.startDate.sendKeys('2016-12');
        ...
        await balanceSheet.toastId.isPresent().then(function(){
             expect(balanceSheet.okToastTitle.getText()).toMatch('Process complet'); 
            expect(balanceSheet.okToastMessage.getText()).toMatch('S-a pornit un process de import. Verificati statusul in zona de notificari');
        });
    });    
